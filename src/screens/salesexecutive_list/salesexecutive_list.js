...
const Component = (props) => {
    ...
    const FetchResults = async () => {
        ...
        await Api.GetSalesExecutivesMulti(query, "SEPhoto").then(async (resP) => {
            if (resP.status) {
                for (let i = 0; i < resP.values.length; i++) {
                    let _SalesExecutive = resP.values[i];
                    let _row = {
                        prop1: _SalesExecutive.Name,
                        prop2: _SalesExecutive.Address,
                        prop3: _SalesExecutive.ContactNumber,
                        prop4: _SalesExecutive.CreatedDate,
                        prop5: _SalesExecutive.ExecId,
                    };

                    _SalesExecutive.SEPhoto &&
                        await Api.GetDocumentSingleMedia(_SalesExecutive.SEPhoto.DocId, true, null).then((resI) => {
