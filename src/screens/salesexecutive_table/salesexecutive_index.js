...
const columns = [
    { headerName: "ExecId", field: "ExecId", flex: 1 },
    { headerName: "Name", field: "Name", flex: 1 },
    { headerName: "ContactNumber", field: "ContactNumber", flex: 1 },
    { headerName: "Address", field: "Address", flex: 1 },
    { headerName: "Attribute5", field: "Attribute5", flex: 1 },
    { headerName: "Attribute1", field: "Attribute1", flex: 1 },
];
...
const Component = (props) => {
    ...
    const FetchResults = async () => {
        ...
        await Api.GetSalesExecutivesCount(query)
            .then(async (res) => {
                if (res.status) {
        ...
        await Api.GetSalesExecutivesMulti(query, "")
            .then(async (res) => {
                if (res.status) {
                    _rows = res.values || [];
                    if (_rows.length > 0) {
    ...
    const OnCloseClicked = async (e) => {
        if (e) {
            const rslt = await Api.SetSalesExecutiveSingle({ ExecId: deletedId, Deleted: true });
            if (rslt.status) {
                setInitialize(true);
    ...
    const OnActionClicked = (id, type) => {
        let _route;
        if (type === 'edit') _route = `/SalesExecutives/edit/${id}`;
        if (type === 'view') _route = `/SalesExecutives/view/${id}`;
    ...
    return (
        ...
        <>
            <Container {...props}>
                    ...
                    <Stack direction="row">
                        <Grid container sx={{ justifyContent: 'flex-end' }}>
                            ...
                            <IconButton
                                ...
                                onClick={() => NavigateTo("/SalesExecutives/create")}
                            >
                                <AddBoxIcon />
                            </IconButton>
                        </Grid>
                ...
                <Box style={{ width: '100%' }}>
                    <DataTable keyId={'ExecId'} columns={columns} rowsCount={rowsCount} rows={rows} sortBy={sortBy} pageInfo={pageInfo} onActionClicked={OnActionClicked}
