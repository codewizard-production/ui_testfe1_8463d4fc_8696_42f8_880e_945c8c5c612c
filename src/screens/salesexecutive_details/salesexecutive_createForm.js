...
const Component = (props) => {
    ...
    const OnSubmit = async () => {
        let rslt, data, prodImages, salesExecutiveId, numfields;
        const mapItems = MapItems;

        let salesexecutive = row['salesexecutive'];

        numfields = Helper.GetAllNumberFields(salesexecutive);
        if (numfields.length > 0) Helper.UpdateNumberFields(salesexecutive, numfields);


        // Add Or Update SalesExecutive
        rslt = await Support.AddOrUpdateSalesExecutive(salesexecutive, ['SEPhoto']);
        if (rslt.status) {
            salesExecutiveId = rslt.id;
        } else { return; }

        for (let i = 0; i < mapItems.length; i++) {
            // Add or Update the SalesExecutive and navigation entity if it is deos not exist
            let navItem = salesexecutive.find(x => x.uicomponent === mapItems[i].uicomponent);
        ...
        global.AlertPopup("success", "SalesExecutive is created successfully!");
        setShowUpdate(false);
        NavigateTo(-1);
    }
