...
const Component = (props) => {
    ...
    const OnSubmit = async () => {
        let rslt, data, changes = [], salesExecutiveId, salesexecutive, numfields, mapItem;

        salesexecutive = row['salesexecutive'];
        salesExecutiveId = row['salesexecutive'].find((x) => x.type === 'keyid').value;
                ...
                if (changes.length > 0) {
                    ...
                    if (tmp.length > 0) {
                        ...
                        if (rslt.status) {
                            ...
                            if (Helper.IsNullValue(mapItem.navpropname)) salesExecutiveId = rslt.id;
                            
                            const mapPropKey = salesexecutive.find(x => x.uicomponent === mapItem.uicomponent).key;
                            data = [
                                { key: "ExecId", value: parseInt(salesExecutiveId) },
                                { key: mapPropKey, value: parseInt(rslt.id) }
                            ];
                            rslt = await Support.AddOrUpdateSalesExecutive(data, dropDownOptions);
        ...
        // Add or Update SalesExecutive SEPhoto
        changes = TrackChanges('salesexecutive');
        if (changes.length > 0 && changes.indexOf('SEPhoto') > -1) {
            data = salesexecutive.find((x) => x.key === 'SEPhoto');
            rslt = await Support.AddOrUpdateDocument(data);
            if (rslt.status) {
                let newImageId = parseInt(rslt.id);
                data = [
                    { key: "ExecId", value: parseInt(salesExecutiveId) },
                    { key: "SalesExecutiveSEPhoto", value: newImageId }
                ];
                rslt = await Support.AddOrUpdateSalesExecutive(data);
                if (!rslt.status) return;

                let newValues = salesexecutive.find((x) => x.key === 'SEPhoto').value;
                newValues = { ...newValues, DocId: newImageId };
                row['salesexecutive'].find((x) => x.key === 'SEPhoto').value = newValues;
                                // Update Back for next tracking purpose
                UpdateBackUp('salesexecutive');
        ...
        // Update root entity
        mapItem = MapItems.find(x => x.uicomponent === 'salesexecutive');
        changes = TrackChanges(mapItem.uicomponent);
        if (changes.length > 0) {
            let tmp = changes.filter((x) => mapItem.exclude.indexOf(x) === -1);
            if (tmp.length > 0) {
        ...
        global.AlertPopup("success", "SalesExecutive is updated successfully!");
        setShowUpdate(false);
        NavigateTo(-1);
    }
