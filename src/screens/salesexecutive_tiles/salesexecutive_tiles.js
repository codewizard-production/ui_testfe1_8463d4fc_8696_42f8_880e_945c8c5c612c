...
const Component = (props) => {
    ...
    const FetchResults = async () => {
        ...
        let _rows = [];
        await Api.GetSalesExecutivesMulti(
            query, 
            "SEPhoto"
            )
            .then(async (res) => {
                if (res.status) {
                    ...
                    for (let i = 0; i < _rows.length; i++) {
                        ...
                        _rows[i].SEPhoto &&
                            await Api.GetDocumentSingleMedia(_rows[i].SEPhoto.DocId, true, "image/jpeg").then((rslt) => {
                                _rows[i].SalesExecutiveSEPhotoData = rslt.values;
                            })
                                            }
                } else {
                    console.log(res.statusText);
